...
function Coursecollege()
{
  ...
  async function fetchData() {
    var apiavail=false;
    const request = await fetch(`http://127.0.0.1:8000/coursecollege-list/`)
      .then(response => {
        if(response.ok)
      {
        console.log("here")
...
if(api)
  ...
  return(
    ...
    <h1>PROFESSIONAL COURSE CONTAINING COLLEGES IN INDIA</h1>
    </div>
    {apiavail ? (
        <>
        <p>{api}</p>
